...
class Expenses():
    ...
    def categorize_set_comprehension(self):
        neccessary_expenses = {x for x in self.list 
                                ...
                                or x.category =="Utilities" or x.category =="Mortagage"}
        food_expenses = {x for x in self.list
                            if x.category=="Groceries"or x.category =="Eating out"}
        unnecessary_expenses = set(self.list) - neccessary_expenses - food_expenses
        return [neccessary_expenses,food_expenses,unnecessary_expenses]
